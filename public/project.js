...
function ppmLineGraph(){
    ...
    d3.csv("Data/annual_co2_ppm.csv", function(dataset){
        ...
        var yScale = d3v5.scaleLinear()
                    .domain([d3.min(dataset, function(d) { return d.average_co2_ppm; }), d3.max(dataset, function(d) { return d.average_co2_ppm; })])
        ...
        var line = d3v5.line()
                ...
                .y(function(d) { return yScale(d.average_co2_ppm); })


                
        var svg = d3v5.select("#ppmLineGraph")               
                        .append("div")
        ...
        var colour = ["#f7fcfd","#e0ecf4","#bfd3e6","#9ebcda","#8c96c6","#8c6bb1","#88419d","#810f7c","#4d004b"];
        ...
        svg.selectAll("dot")
            ...
            .attr("cy", function(d) { return yScale(d.average_co2_ppm); })
            ...
            .on("mouseover", function(d) {
              ...
              div.html(
                ...
                "<br/><strong>Co2 emissions:</strong> " + d.average_co2_ppm + " Parts per million")     
...
{  
    ...
    d3.csv("Data/global_temperature_indic.csv", function(data) {
                ...
                var allGroup = ["Land Max Temperature", "Land And Ocean Average Temperature"]

                var svg = d3v5.select("#tempSelect")
                                .append("div")
                                .classed("all_svg_container", true) 
                                .append("svg")
                                .attr("preserveAspectRatio", "xMinYMin meet")
                ...
                var yScale = d3v5.scaleLinear()
                                .domain([d3.min(data, function(d) { return d["Land Max Temperature"]; }), d3.max(data, function(d) { return d["Land And Ocean Average Temperature"]; })])
                                .range([height - padding, 0]);
                    

                d3v5.select("#tempSelectButton")
                    .selectAll('myoptions')
                    .data(allGroup)
                    .enter()
                    .append('option')
                ...
                var line = svg
                        ...
                        .append("path")
                            ...
                            .attr("d", d3v5.line()
                                            ...
                                            .x(function(d) { return xScale(+d.Year) })
                                            .y(function(d) { return yScale(+d["Land Max Temperature"]) })
                                            )
                            .attr("stroke", function(d){ return myColor("LandMaxTemperature") })
...
{  
    ...
    d3.csv("Data/global_temp_avg_country_wide_form.csv", function(data) {
                ...
                var svg = d3v5.select("#tempCountrySelect")
                                .append("div")
                                .classed("all_svg_container", true) 
                                .append("svg")
                                .attr("preserveAspectRatio", "xMinYMin meet")
                ...
                var yScale = d3v5.scaleLinear()
                                .range([height - padding, 0])
                                .domain([d3v5.min(data, function(d) { return d["Australia"]  }), d3v5.max(data, function(d) { return d["Australia"]  }) ]);
                    

                d3v5.select("#tempCountrySelectButton")
                ...
                var line = svg
                        ...
                        .append("path")
                            ...
                            .attr("stroke", function(d){ return myColor(d["Australia"]) })
                            .style("stroke-width", 4)
                            .style("fill", "none");
                ...
                function update(selectedGroup) {
                            ...
                            yScale.domain([d3v5.min(data, function(d) { return d[selectedGroup]  }), d3v5.max(data, function(d) { return d[selectedGroup]  }) ]);
    ...
    d3.csv("Data/all_years_nations_4_treemap.csv", function(data){
        ...
        d3plus.viz()
            ...
            .ui([
                {
                    "method":"size",
                    "value": ["Cement","Gas Flaring","Gas Fuel","Liquid Fuel", "Solid Fuel", "Per Capita", "Total"]
                }
            ])
            .tooltip({
                "share": true, 
...
function BarGraphWaterTop4()
{
        d3.csv("Data/water_poll_Ind_top_4.csv", function(data) {
            data.forEach(function(d) {
                d3.keys(d).forEach(function(k){
                    if((k != "Corporation") && (k != "image_loc") && (k != "Market_Capitalisation") && (k != "Amount_of_toxic") && (k!= "Industry")){
                        d[k] = +d[k]
                    }
                })
            });
            ...
            var svg = d3v5.select("#water_bartop4")
                        .append("div")
                        .classed("all_svg_container", true) 
                        .append("svg")
            ...
            var x = d3v5.scaleBand()
                        .range([ 0, width ])
                        .domain(data.map(function(d) { return d.Corporation; }))
            ...
            var y = d3v5.scaleLinear()
                        .domain([0, d3v5.max(data, function(d) { return d.Toxic_hazard; })])
                        .range([height - padding, 0])
                        svg.append("g")
                            .attr("transform", "translate(" + padding + ",0)")
            ...
            defs.selectAll(".logo-pattern")
                ...
                .enter()
                .append("pattern")
                .attr("id", function (d){ return d.Corporation.toLowerCase().replace(" ", "-")})
                .attr("height", "100%")
                .attr("width", "100%")
                .attr("patternContentUnits", "userSpaceOnUse")
                    ...
                    .attr("xlink:href", function (d){return d.image_loc});
            ...
            var tip = d3.tip()
                    ...
                    .offset([-10, 10])
                    .html(function(d) {
                      return "<strong>Toxins Emitted in Water:</strong> " + d.Amount_of_toxic + "</br>"
                            + "<strong>Market Cap:</strong> " + d.Market_Capitalisation + "</br>"
                            +"<strong>Year Established:</strong> " + d.Year_Established + "</br>"
                            +"<strong>Industry:</strong> " + d.Industry + "</br>"
                            +"<strong>Porportion in Top 4:</strong> " + d.Percentage + "%" +"</br>";
                    });

            
            svg.call(tip);

            svg.selectAll("#water_bartop4")
                .data(data)
                .enter()
                .append("rect")
                    .attr("x", function(d) { return x(d.Corporation) ; })
                    .attr("y", function(d) { return y(d.Toxic_hazard); })
                    .attr("width", x.bandwidth())
                    .attr("height", function(d) { return  (height + padding) - y(d.Toxic_hazard); })
                    .attr("fill", function (d) { return "url(#" + d.Corporation.toLowerCase().replace(" ", "-") + ")"})
                    .on("mouseover", function(d) {
                        tip.show(d, this);
                    })
...
function BarGraphAirTop4()
{
        d3.csv("Data/air_poll_Ind_top_4.csv", function(data) {
            ...
            var svg = d3v5.select("#air_bartop4")
                        .append("div")
                        .classed("all_svg_container", true) 
                        .append("svg")
                        .attr("preserveAspectRatio", "none")
            ...
            var tip = d3.tip()
                ...
                .offset([-10, 10])
                .html(function(d) {
                    return "<strong>Toxins Emitted in Air:</strong> " + d.Amount_of_toxic + "</br>"
                        + "<strong>Market Cap:</strong> " + d.Market_Capitalisation + "</br>"
                        +"<strong>Year Established:</strong> " + d.Year_Established + "</br>"
                        +"<strong>Industry:</strong> " + d.Industry + "</br>"
                        +"<strong>Porportion in Top 4:</strong> " + d.Percentage + "%" +"</br>";
...
function opinionBar()
{
    ...
    d3.csv("Data/choices_made_to_protect_environ.csv", function(data) {
        ...
        var svg = d3v5.select("#opinion_bar")
                        .append("div")
                        .classed("all_svg_container", true) 
                        .append("svg")
                        .attr("preserveAspectRatio", "none")
        ...
        x.domain(data.map(function(d) { return d.Action; }));
        y.domain([0, 40]);

        
        svg.selectAll("bar_op")
            .data(data)
            .enter().append("rect")
            .attr("class", "bar_op")
            .attr("x", function(d) { return x(d.Action); })
            .attr("width", x.bandwidth())
            .attr("y", function(d) { return y(d.Percent_of_respondent); })
            .attr("height", function(d) { return height - y(d.Percent_of_respondent); })
            .attr("fill", "#02c39a")
